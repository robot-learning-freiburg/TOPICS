    ...
    def forward(self, logit, label, logit_old=None):
        ...
        if logit_old is None:
            if len(label.shape) == 3:
                # target: [N, C, H, W], convert to one hot
            ...
            if len(label.shape) == 3:
                # target: [N, C, H, W]
                target = torch.zeros_like(logit).float().to(logit.device)
                target[:, 1:logit_old.shape[1]] = logit_old.sigmoid()[:, 1:] # copy over prediction of old model
                for cls_idx in label.unique():
                    if cls_idx in self.ignore_indexes:
                        continue
                    target[:, int(cls_idx)] = (label == int(cls_idx)).float()
...
class WBCELoss(nn.Module):
    ...
    def forward(self, logit, label):
        # logit:     [N, |Ct|, H, W]
        # label:     [N, H, W]
        # reshape logits to only take new ones! (sigmoid with bce)

        logit = logit[:, -self.n_new_classes:] # only new classes
        ...
        for cls_idx in label.unique():
            if cls_idx in [0, self.ignore_index]:
                continue
            target[:, int(cls_idx) - self.n_old_classes] = (label == int(cls_idx)).float() # get target only of new classes
