...
def find_median(train_loader, device, logger, model_old, nb_current_classes, threshold, mode="probability"):
    ...
    """
    if mode == "entropy":
        max_value = torch.log(torch.tensor(nb_current_classes).float().to(device))
        nb_bins = 100
    else:
        max_value = 1.0
    ...
    for cur_step, (images, labels) in enumerate(train_loader):
        ...
        outputs_old, features_old = model_old(images, ret_intermediate=False)
    ...
    thresholds = torch.zeros(nb_current_classes, dtype=torch.float32).to(device)
...
def features_distillation(
    ...
    factor = len(list_attentions_a)
    if final_layer_only:
        factor = 1
    elif internal_layers_only:
        factor -= 1
