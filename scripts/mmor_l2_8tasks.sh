#!/usr/bin/env bash
task="configs/mmor/task_files/l2_increment_8tasks"
method="FT"
base_setting="--step 0 --epoch 60 --lr 0.02"

### PLOP 
method="PLOP"
run_setting="--sample_num 2 --dataset mmor --batch_size 24 --autocast --crop_size 512"
...
weights="--step_prev_ckpt output/mmor_l2_increment_8tasks-ov/eucl_rn_v4.0_ft_sq_v2_60ep/step0/model_final.pth"
...
base_setting="--step 0 --epoch 60 --lr 0.02"
run_setting="--sample_num 2 --dataset mmor --batch_size 24 --autocast"
python -m torch.distributed.launch --nproc_per_node=2 --master_port 4477 run.py --name ${name} --task ${task} --method ${method} ${base_setting} ${run_setting}
...
weights="--step_prev_ckpt output/mmor_l2_increment_8tasks-ov/eucl_rn_v4.0_ft_lr2_60ep/step0/model_final.pth"
for t in 1 2 3 4 5 6 7; do
...
run_setting="--sample_num 2 --dataset mmor --batch_size 32 --autocast"
method="DKD"
name="dkd-v4-60ep-t5"
